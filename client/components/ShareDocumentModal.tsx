...
export default function ShareDocumentModal({ documentId, isOpen, onClose, onSuccess }: ShareDocumentModalProps) {
    ...
    // Fetch current collaborators when opened
    useEffect(() => {
        if (!isOpen) {
            setQuery('');
            setResults([]);
            ...
            <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-xl animate-in fade-in zoom-in duration-200">
                ...
                <div className="p-6">
                    ...
                    {collaborators.length > 0 && (
                        <div className="mb-6">
                            <h3 className="text-sm font-medium text-slate-700 mb-2">Current Collaborators</h3>
                            <ul className="space-y-2 max-h-40 overflow-y-auto pr-1">
                                {collaborators.map((c: any) => (
                                    <li key={c.userId} className="flex justify-between items-center p-2 rounded-lg bg-slate-50 border border-slate-100">
                                        <div>
                                            <p className="text-sm font-medium text-slate-900">{c.user?.displayName} {c.role === 'OWNER' && '(Owner)'}</p>
                                            <p className="text-xs text-slate-500">{c.user?.email}</p>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <span className="text-[10px] font-bold tracking-wider bg-slate-200 text-slate-700 px-2 py-1 rounded-full uppercase">{c.role}</span>
                    ...
                    {!selectedUser ? (
                        <div>
                            ...
                            <input
                                ...
                                className="w-full px-4 py-2 border border-slate-300 rounded-lg text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition"
                                autoFocus
                                onClick={(e) => e.stopPropagation()}
                            />

                            <div className="mt-4 max-h-48 overflow-y-auto">
                                {isLoading && <div className="text-sm text-center py-4 text-slate-500">Searching...</div>}

                                {!isLoading && query.length >= 2 && results.length === 0 && (
                                    <div className="text-sm text-center py-4 text-slate-500">No users found.</div>
                                )}
                                    ...
                                    <ul className="space-y-2">
                                        {results.map(user => (
                                            <li
                                                ...
                                                className="p-3 rounded-lg border border-slate-100 hover:border-indigo-200 cursor-pointer transition flex justify-between items-center"
                                            >
                                                <div>
                        ...
                        <div>
                            <div className="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-100 mb-4">
                                <div>
                                    <p className="text-sm font-medium text-slate-900">{selectedUser.displayName}</p>
                            ...
                            <div className="mb-4">
                                ...
                                <p className="text-sm text-slate-500 bg-white border border-slate-200 p-3 rounded-lg shadow-sm">
