...
export default function ReviewPane({ editor, accessMode, onClose }: ReviewPaneProps) {
    ...
    return (
        <div className="flex flex-col h-full bg-white border-l w-80 shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)] dark:bg-slate-900 dark:border-slate-700">
            {/* ── Header ──────────────────────────────────────────────────── */}
            <div className="flex items-center justify-between px-4 py-3 border-b bg-slate-50 dark:bg-slate-800 dark:border-slate-700">
                <div className="flex items-center gap-2">
                    <span className="text-red-500 font-bold text-lg">┃</span>
                    <div>
                        <h2 className="text-sm font-bold text-slate-900 tracking-tight dark:text-white">CHANGES</h2>
                        <p className="text-[10px] text-slate-400 uppercase tracking-wider dark:text-slate-500">
                ...
                <button
                    onClick={onClose}
                    className="p-1 text-slate-400 hover:text-slate-700 rounded hover:bg-slate-200 transition dark:text-slate-500 dark:hover:text-slate-300 dark:hover:bg-slate-700"
                >
                    <X size={16} />
                </button>
            </div>
            ...
            <div className="px-3 py-2 border-b space-y-2 bg-slate-50/50 dark:bg-slate-800/50 dark:border-slate-700">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                        <button
                            ...
                            className="p-1 rounded hover:bg-slate-200 text-slate-500 disabled:opacity-30 transition dark:hover:bg-slate-700 dark:text-slate-400"
                            title="Previous Change"
                        >
                            <ChevronLeft size={14} />
                        </button>
                        <span className="text-[10px] text-slate-400 font-medium min-w-[3rem] text-center tabular-nums dark:text-slate-500">
                ...
                {/* Row 2: Filters */}
                <div className="flex items-center gap-2">
                    <Filter size={10} className="text-slate-400 shrink-0 dark:text-slate-500" />
                    <select
                        value={filterType}
                        onChange={(e) => setFilterType(e.target.value as any)}
                        className="text-[10px] bg-white border border-slate-200 rounded px-1.5 py-0.5 text-slate-600 flex-1 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-300"
                    >
                ...
                {canReview && filteredChanges.length > 0 && (
                    <div className="flex gap-1.5">
                        <button
                            ...
                            className="flex-1 flex items-center justify-center gap-1 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded text-[10px] font-bold uppercase hover:bg-emerald-100 transition dark:bg-emerald-900/20 dark:text-emerald-300 dark:border-emerald-800 dark:hover:bg-emerald-900/30"
                        >
                            <CheckCheck size={11} /> Accept All
                        </button>
                        <button
                            onClick={handleRejectAll}
                            className="flex-1 flex items-center justify-center gap-1 py-1 bg-red-50 text-red-700 border border-red-200 rounded text-[10px] font-bold uppercase hover:bg-red-100 transition dark:bg-red-900/20 dark:text-red-300 dark:border-red-800 dark:hover:bg-red-900/30"
                        >
            ...
            <div className="flex-1 overflow-y-auto">
                {filteredChanges.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-2 px-6 dark:text-slate-500">
                        <CheckCheck size={32} className="text-slate-300 dark:text-slate-600" />
                            ...
                            return (
                                <div
                                    ...
                                    className={`cursor-pointer transition-colors ${isActive
                                        ...
                                        : 'hover:bg-slate-50 border-l-2 border-l-transparent dark:hover:bg-slate-800/50'
                                        }`}
                                >
                                    {/* Card Header */}
                                    <div className="flex items-center justify-between px-3 pt-2.5 pb-1">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[10px] text-slate-400 dark:text-slate-500">{relativeTime(change.timestamp)}</p>
                                            </div>
                                        </div>
                                        <span
                                    ...
                                    {canReview && change.userId !== currentUser?.userId && (
                                        <div className="flex border-t border-slate-100 divide-x divide-slate-100 dark:border-slate-800 dark:divide-slate-800">
                                            <button
                                                ...
                                                className="flex-1 flex items-center justify-center gap-1 py-1.5 text-[10px] font-bold text-emerald-600 hover:bg-emerald-50 transition uppercase dark:text-emerald-400 dark:hover:bg-emerald-900/20"
                                            >
                                            ...
                                            <button
                                                ...
                                                className="flex-1 flex items-center justify-center gap-1 py-1.5 text-[10px] font-bold text-red-600 hover:bg-red-50 transition uppercase dark:text-red-400 dark:hover:bg-red-900/20"
                                            >
                                                <X size={12} /> Reject
                                            </button>
                                        </div>
